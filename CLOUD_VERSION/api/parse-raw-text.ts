...
function parseRawText(rawText: string) {
  ...
  for (const line of lines) {
    ...
    // 优先匹配明确的分类标签（如"心情："、"学习："等）
    if (lowerLine.startsWith('心情：') || lowerLine.startsWith('心情:')) {
      result.mood_text = line.replace(/^心情[：:]\s*/, '');
    }
    else if (lowerLine.startsWith('健身：') || lowerLine.startsWith('健身:') || 
             lowerLine.startsWith('运动：') || lowerLine.startsWith('运动:')) {
      result.fitness_text = line.replace(/^(健身|运动)[：:]\s*/, '');
    }
    else if (lowerLine.startsWith('学习：') || lowerLine.startsWith('学习:')) {
      result.study_text = line.replace(/^学习[：:]\s*/, '');
    }
    else if (lowerLine.startsWith('工作：') || lowerLine.startsWith('工作:')) {
      result.work_text = line.replace(/^工作[：:]\s*/, '');
    }
    else if (lowerLine.startsWith('灵感：') || lowerLine.startsWith('灵感:')) {
      result.inspiration_text = line.replace(/^灵感[：:]\s*/, '');
    }
    // 如果没有明确的分类标签，则使用关键词匹配
    else {
      // 心情相关关键词
