...
import { analyzeMoodData } from '../backend/ai-service.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const item of data) {
      ...
      try {
        // 使用AI分析心情数据
        const aiAnalysis = await analyzeMoodData(description);
        ...
        if (existing.rows && existing.rows.length > 0) {
          await turso.execute({
            sql: `UPDATE simple_records SET 
             ...
             WHERE id = ?`,
            args: [description, aiAnalysis.mood_emoji, aiAnalysis.mood_event, aiAnalysis.mood_score, aiAnalysis.mood_category, existing.rows[0][0]]
          });
          results.push({ id: existing.rows[0][0], date, description, ...aiAnalysis, action: 'update' });
        } else {
          await turso.execute({
            sql: `INSERT INTO simple_records (date, mood_description, mood_emoji, mood_event, mood_score, mood_category) 
             VALUES (?, ?, ?, ?, ?, ?)`,
            args: [date, description, aiAnalysis.mood_emoji, aiAnalysis.mood_event, aiAnalysis.mood_score, aiAnalysis.mood_category]
          });
    ...
    console.error('批量处理心情数据失败:', error);
    res.status(500).json({ 
      success: false, 
      message: '批量处理心情数据失败',
      error: error.message 
