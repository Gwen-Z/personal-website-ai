...
function RadarOverview({ data, from, to, setFrom, setTo, onQuery, loading }: ComponentProps) {
  return (
    <Card>
      <SectionHeader title="综合能力雷达图" from={from} to={to} onSetFrom={setFrom} onSetTo={setTo} onQuickSelect={()=>{}} onQuery={onQuery} />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="h-72 rounded-lg bg-slate-50 flex items-center justify-center text-slate-400">
          {loading ? '加载中...' : '雷达图区域'}
        </div>
        <div className="grid grid-cols-5 gap-3">
          {['心情','生活','学习','工作','创意'].map((k,i)=>(
            <div key={k} className="rounded-lg bg-slate-50 p-3 text-center">
              <div className="text-2xl font-bold text-indigo-600">{[72,81,85,78,68][i]}</div>
...
function EmotionTrend({ data, from, to, setFrom, setTo, onQuery, loading }: ComponentProps) {
  // 筛选情绪相关数据
...
function LifeTimeline({ data, from, to, setFrom, setTo, onQuery, loading }: ComponentProps) {
  // 筛选生活相关数据
...
function StudyTimeDist({ data, from, to, setFrom, setTo, onQuery, loading }: ComponentProps) {
  // 筛选学习相关数据
...
function WorkCompletion({ data, from, to, setFrom, setTo, onQuery, loading }: ComponentProps) {
  // 筛选工作相关数据
...
function InspirationNotes({ data, from, to, setFrom, setTo, onQuery, loading }: ComponentProps) {
  // 筛选灵感相关数据
  ...
  return (
    <div className="grid grid-cols-[220px_1fr] gap-6">
      ...
      <section className="space-y-6">
        {active==='radar' && <RadarOverview data={data} from={from} to={to} setFrom={setFrom} setTo={setTo} onQuery={loadData} loading={loading} />}
        {active==='emotion' && <EmotionTrend data={data} from={from} to={to} setFrom={handleSetFrom} setTo={handleSetTo} onQuery={loadData} loading={loading} />}
        {active==='life' && <LifeTimeline data={data} from={from} to={to} setFrom={handleSetFrom} setTo={handleSetTo} onQuery={loadData} loading={loading} />}
        {active==='study' && <StudyTimeDist data={data} from={from} to={to} setFrom={handleSetFrom} setTo={handleSetTo} onQuery={loadData} loading={loading} />}
        {active==='work' && <WorkCompletion data={data} from={from} to={to} setFrom={handleSetFrom} setTo={handleSetTo} onQuery={loadData} loading={loading} />}
        {active==='inspiration' && <InspirationNotes data={data} from={from} to={to} setFrom={handleSetFrom} setTo={handleSetTo} onQuery={loadData} loading={loading} />}
      </section>
    </div>
  )
}